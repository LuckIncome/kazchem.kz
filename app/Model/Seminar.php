...
class Seminar extends AppModel{
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/seminars/' . $fileName;
		$path_th = WWW_ROOT . 'img/seminars/thumbs/' . $fileName;
	...
	public function customUploadImg2($file = array()){
		if(!is_uploaded_file($file['img2']['tmp_name'])){
			return false;
		}
		...
		$path = WWW_ROOT . 'img/seminars/' . $fileName;
		$path_th = WWW_ROOT . 'img/seminars/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img2']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 844, 250, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/seminars/' . $name)){
