...
class StaticPage extends AppModel{
 ...
 //        ),
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/static_pages/' . $fileName;
		$path_th = WWW_ROOT . 'img/static_pages/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 200, 200, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/static_pages/' . $name)){
