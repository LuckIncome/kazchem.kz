...
class ProductImagesController extends AppController{
	public $uses = array('ProductImage', 'Partner');
	public function admin_index(){
		$data = $this->ProductImage->find('all', array(
	...
	public function index(){
		$data = $this->ProductImage->find('all');
	...
	public function admin_add($id){
		if($this->request->is('post')){
			...
			$data = $this->request->data['ProductImage'];
	...
	public function admin_edit($id){
		if(is_null($id) || !(int)$id || !$this->ProductImage->exists($id)){
		...
		if($this->request->is(array('post', 'put'))){
			...
			$data1 = $this->request->data['ProductImage'];
			if(empty($data1['img']['name']) || !$data1['img']['name']){
				unset($data1['img']);
			}
			if($this->ProductImage->save($data1)){
				$this->Session->setFlash('Сохранено', 'default', array(), 'good');
				return $this->redirect($this->referer());
			}else{
				$this->Session->setFlash('Ошибка', 'default', array(), 'bad');
